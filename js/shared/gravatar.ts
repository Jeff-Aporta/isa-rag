/**
 * Gravatar helper — usa https://www.gravatar.com/avatar/<md5(email)>?d=identicon
 * No requiere API key, no requiere auth.
 *
 * Para usuarios sin email publico (como 'jagudeloe' que es solo un username)
 * se usa el username como semilla hash → identicon consistente.
 *
 * El hash MD5 sale de una implementacion en JS puro (no requiere dependencias).
 */
function md5(str: string): string {
  // MD5 estandar (RFC 1321) — pequeno y rapido. Suficiente para Gravatar.
  ...
  function sb(s: string): number[] {
    const n = s.length;
    const w: number[] = [];
    for (let i = 0; i < n; i++) w[i >> 2] = (w[i >> 2] || 0) | (s.charCodeAt(i) << ((i % 4) * 8));
...
/** Detecta si el input es un email (contiene @) o un username simple. */
function normalizeSeed(input: string): string {
  const s = (input || "").trim().toLowerCase();
  if (!s) return "anonymous@isa-rag.local";
  if (s.includes("@")) return s;
  // Sin email: usar username como semilla. Gravatar devuelve el identicon del hash.
  return `${s}@isa-rag.local`;
}

/**
 * URL Gravatar. d=identicon para que SIEMPRE se muestre un avatar generado
 * (aunque el email no este registrado en Gravatar). s= tamano en px.
 */
export function gravatarUrl(input: string, size = 40): string {
